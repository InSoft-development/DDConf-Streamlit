		...
		if st.session_state.dd104m['ld-editor-flag']:
			...
			if loadout['fcount'] <= 0:
				with cont:
					...
					col2.checkbox(label="Добавить выбор архивных файлов", value=st.session_state.dd104m['ld-archive-use-flag'][0] if 0 in st.session_state.dd104m['ld-archive-use-flag'].keys() else False, on_change=checker, kwargs={'i':0}, key=f'ld-archive-use-cbox-0')
				...
				with cont:
					...
					for i in range(1, loadout['fcount']+1):
						...
						col2.checkbox(label="Добавить выбор архивных файлов", value=(st.session_state.dd104m['ld-archive-use-flag'][i] if i in st.session_state.dd104m['ld-archive-use-flag'].keys() else False), on_change=checker, kwargs={'i':i}, key=f'ld-archive-use-cbox-{i}')
...
def draw_table_status():
	if 'active_ld' in st.session_state.dd104m.keys() and st.session_state.dd104m['active_ld']:
		...
		options = [f"{i}: {[f['savename']+' ('+f['savetime']+')' for f in filelist if f['filename'] == ldlist[i]][0] if [f['savename']+' ('+f['savetime']+')' for f in filelist if f['filename'] == ldlist[i]] else 'Файл не назначен'}" for i in range(1, st.session_state.dd104m['active_ld']['fcount']+1)] 
